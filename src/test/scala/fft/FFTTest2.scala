    ...
    "FFTEngine with fixedpoint should calculate RFFT repeatly" in {
        ...
        test(new FFTEngine).withAnnotations(Seq(VerilatorBackendAnnotation)) { dut =>
            ...
            def complexUInt2Complex(dataIn : UInt) : Complex = {
                val dataIPre = dataIn((fftDataWidth + 2) * 2 - 1, fftDataWidth + 2).litValue.toInt
                val dataRPre = dataIn(fftDataWidth + 1, 0).litValue.toInt
                val dataISign = dataIn((fftDataWidth + 2) * 2 - 1).litValue.toInt
                val dataRSign = dataIn(fftDataWidth + 1).litValue.toInt
                val dataI = if(dataISign == 1) dataIPre - pow(2, fftDataWidth + 2).toInt else dataIPre
                val dataR = if(dataRSign == 1) dataRPre - pow(2, fftDataWidth + 2).toInt else dataRPre
            ...
            for(rp <- 0 until RPCnt by 1) {
                ...
                //output data
                for(radix <- 1 until fftLength) { //ignore DC
